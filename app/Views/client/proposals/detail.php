...
<script>
const BASE = '<?= base_url() ?>'; const CSRF = document.querySelector('meta[name=csrf-token]').content;
$('.btn-accept-prop').on('click', function(e) {
  e.preventDefault();
  if (!confirm('Accept this proposal?')) return;
  $.post(`${BASE}portal/proposals/respond/<?= $proposal['id'] ?>`, {action:'accept',csrf_test_name:CSRF}, res => {
    if (res.status==='success') location.reload(); else alert(res.message);
  },'json');
});
$('.btn-reject-prop').on('click', function(e) {
  e.preventDefault();
  if (!confirm('Request a revision?')) return;
  $.post(`${BASE}portal/proposals/respond/<?= $proposal['id'] ?>`, {action:'revision',csrf_test_name:CSRF}, res => {
    if (res.status==='success') location.reload(); else alert(res.message);
  },'json');
});
</script>
