...
<div class="card border-0 shadow-sm"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th class="ps-4">Deliverable</th><th>Milestone</th><th>Owner</th><th>Due</th><th>Status</th><th class="pe-4 text-end">Action</th></tr></thead><tbody><?php if (!$projectId || empty($deliverables)): ?><tr><td colspan="6" class="text-center text-muted py-5"><?= $projectId ? 'No deliverables yet.' : 'Select a project to view deliverables.' ?></td></tr><?php else: ?><?php $colors=['draft'=>'secondary','in_progress'=>'primary','submitted'=>'info','under_review'=>'warning','changes_requested'=>'danger','approved'=>'success','rejected'=>'dark']; foreach ($deliverables as $d): ?><tr data-deliverable-row="<?= $d['id'] ?>"><td class="ps-4"><div class="fw-semibold small"><a href="<?= base_url('admin/deliverables/view/'.$d['id']) ?>" class="text-decoration-none"><?= esc($d['title']) ?></a><?php if ($d['status']==='changes_requested'): ?> <i class="bi bi-exclamation-triangle-fill text-danger" title="Client requested changes"></i><?php endif; ?></div><div class="text-muted" style="font-size:11px">v<?= esc($d['version']) ?><?php $fc = ($fileCounts[$d['id']] ?? 0); if ($fc): ?> · <i class="bi bi-paperclip"></i><?= (int)$fc ?> file<?= $fc > 1 ? 's' : '' ?><?php endif; ?></div></td><td class="small"><?= esc($d['milestone_title'] ?? '—') ?></td><td class="small"><?= esc($d['owner_name'] ?? '—') ?></td><td class="small"><?= $d['due_date'] ? date('d M Y',strtotime($d['due_date'])) : '—' ?></td><td><span class="badge bg-<?= $colors[$d['status']] ?? 'secondary' ?> deliverable-status-badge"><?= ucwords(str_replace('_',' ',$d['status'])) ?></span></td><td class="pe-4 text-end"><a href="<?= base_url('admin/deliverables/view/'.$d['id']) ?>" class="btn btn-xs btn-outline-primary me-1" title="View"><i class="bi bi-eye"></i></a><select class="form-select form-select-sm d-inline-block deliverable-status-select" data-id="<?= $d['id'] ?>" style="width:150px"><?php foreach ($colors as $s => $c): ?><option value="<?= $s ?>" <?= $d['status']===$s?'selected':'' ?>><?= ucwords(str_replace('_',' ',$s)) ?></option><?php endforeach; ?></select></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div></div></div>
<?= $this->endSection() ?>
