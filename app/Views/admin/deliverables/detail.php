          ...
          <?php foreach ($files as $f): ?>
            ...
            <div class="col-6 col-md-4" data-deliverable-file="<?= $f['id'] ?>">
              ...
              <?php elseif ($fIsVideo && $fPlayable): ?>
                <video src="<?= base_url('admin/deliverables/files/'.$f['id']) ?>" class="rounded border w-100" style="aspect-ratio:16/9;object-fit:cover;background:#000" controls preload="metadata" playsinline></video>
              <?php elseif ($fIsVideo): ?>
                <a href="<?= base_url('admin/deliverables/files/'.$f['id'].'?download=1') ?>" class="d-flex flex-column align-items-center justify-content-center border rounded text-decoration-none text-dark p-2" style="aspect-ratio:16/9"><i class="bi bi-film fs-2 text-muted"></i><span class="small text-truncate w-100 text-center"><?= esc($f['original_name']) ?></span><span class="badge bg-light text-dark border mt-1" style="font-size:10px">Download to play</span></a>
              <?php else: ?>
              ...
              <div class="d-flex justify-content-between align-items-center"><span class="text-muted" style="font-size:10px"><?= esc($f['uploader_name'] ?? '') ?></span><span class="d-flex gap-1"><a href="<?= base_url('admin/deliverables/files/'.$f['id'].'?download=1') ?>" class="btn btn-xs text-primary p-0" title="Download" download><i class="bi bi-download"></i></a><?php if (!empty($canManage)): ?><button type="button" class="btn btn-xs text-danger p-0 btn-del-dfile" data-id="<?= $f['id'] ?>" title="Remove"><i class="bi bi-trash"></i></button><?php endif; ?></span></div></div>
            </div>
          <?php endforeach; ?>
