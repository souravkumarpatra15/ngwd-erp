...
const dt = $('#agreementsTable').DataTable({ pageLength: 25, order: [[6,'desc']] });
...
$(document).on('click', '.btn-send-email', function() {
  ...
  $.post(`${BASE}admin/agreements/send-email/${id}`, {csrf_test_name: CSRF}, res => {
...
$(document).on('click', '.btn-send-wa', function() {
  ...
  $.post(`${BASE}admin/agreements/send-whatsapp/${id}`, {csrf_test_name: CSRF}, res => {
    hideLoader(); showToast(res.message, res.status);
  }, 'json');
});
...
$('#ngConfirmYes').off('click').on('click', function() {
  ...
  showLoader('Deleting...');
  $.post(`${BASE}admin/agreements/delete/${delId}`, {csrf_test_name: CSRF}, res => {
    hideLoader(); showToast(res.message, res.status);
