...
const invoicesTable = $('#invoicesTable').DataTable({
  ...
  columns: [
    ...
    { data: 'total', render: (d,t,r) => curSym(r.currency)+parseFloat(d).toLocaleString('en-IN',{minimumFractionDigits:2}) },
    { data: 'paid_amount', render: (d,t,r) => `<span class="text-success">${curSym(r.currency)}${parseFloat(d).toLocaleString('en-IN',{minimumFractionDigits:2})}</span>` },
    { data: 'balance_due', render: (d,t,r) => d > 0 ? `<span class="text-danger fw-semibold">${curSym(r.currency)}${parseFloat(d).toLocaleString('en-IN',{minimumFractionDigits:2})}</span>` : `<span class="text-success">${curSym(r.currency)}0.00</span>` },
