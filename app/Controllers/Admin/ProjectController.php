...
class ProjectController extends BaseController
{
    ...
    private function canViewFinancials(): bool { return $this->pmsAuth->canViewFinancials((string) session()->get('user_role'), (string) session()->get('department')); }
    ...
    public function datatable() {
        ...
        foreach($data as &$row){$stat=$msStats[$row['id']]??null;$row['progress']=($stat&&(int)$stat['total']>0)?(int)round(((int)$stat['done']/(int)$stat['total'])*100):0; if(!$canViewFinancials){unset($row['budget'],$row['advance_paid'],$row['total_paid']);}}unset($row);return $this->response->setJSON(['draw'=>intval($this->request->getGet('draw')),'recordsTotal'=>$total,'recordsFiltered'=>$total,'data'=>$data]);
    }
    ...
    public function store(){ if(!$this->pmsAuth->isPrivilegedInternal((string)session()->get('user_role')) && strtolower((string)session()->get('user_role'))!=='manager') return redirect()->to('admin/projects')->with('error','Access denied.'); if(!$this->validate(['client_id'=>'required|integer','name'=>'required|min_length[2]','type'=>'required']))return redirect()->back()->withInput()->with('errors',$this->validator->getErrors());$data=array_merge($this->request->getPost(),['project_number'=>$this->generateNumber('PROJ',$this->projectModel),'created_by'=>session()->get('user_id')]);unset($data['csrf_test_name']);$id=$this->projectModel->insert($data);$this->logActivity('projects',$id,'created','Project: '.$data['name']);try{$client=(new ClientModel())->find((int)($data['client_id']??0));if($client&&trim((string)($client['whatsapp']??''))!=='')(new WhatsAppNotificationService())->projectCreated((string)$client['whatsapp'],(string)$client['name'],(string)$data['name']);}catch(\Throwable $e){log_message('error','projectCreated WA failed: '.$e->getMessage());}$userId=(int)session()->get('user_id');if($userId>0)(new ProjectMemberModel())->addMember((int)$id,$userId,'project_manager','manage');return redirect()->to("admin/projects/$id")->with('success','Project created!'); }
    public function show($id){$project=$this->projectModel->getWithClient($id);if(!$project)return redirect()->to('admin/projects');if(!$this->pmsAuth->canViewProjectScoped((string)session()->get('user_role'),(int)session()->get('user_id'),(int)$id))return redirect()->to('admin/projects')->with('error','You are not assigned to this project.');if($this->request->getGet('dashboard')==='1')return view('admin/projects/dashboard',['title'=>$project['name'].' — PMS Dashboard','project'=>$project,'stats'=>$this->getProjectDashboardStats((int)$id),'members'=>(new ProjectMemberModel())->getByProject((int)$id),'activities'=>(new ActivityModel())->where('module','projects')->where('module_id',$id)->orderBy('created_at','DESC')->limit(12)->findAll(),'canViewFinancials'=>$this->canViewFinancials()]);return view('admin/projects/show',['title'=>$project['name'],'project'=>$project,'milestones'=>(new MilestoneModel())->where('project_id',$id)->orderBy('sort_order')->findAll(),'tasks'=>(new TaskModel())->where('project_id',$id)->orderBy('created_at','DESC')->findAll(),'documents'=>(new DocumentModel())->where('project_id',$id)->findAll(),'activities'=>(new ActivityModel())->where('module','projects')->where('module_id',$id)->orderBy('created_at','DESC')->limit(20)->findAll(),'members'=>(new ProjectMemberModel())->getByProject((int)$id),'progress'=>$this->projectModel->getProgress($id),'canViewFinancials'=>$this->canViewFinancials()]);}
