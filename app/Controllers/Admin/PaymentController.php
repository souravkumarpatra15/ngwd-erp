...
class PaymentController extends BaseController
{
    ...
    public function datatable()
    {
        ...
        $result = $this->pm->getDataTable(
            is_array($search) ? trim((string) ($search['value'] ?? '')) : '',
    ...
    public function store()
    {
        ...
        try {
            ...
            $project = null;
            if ($projectId) {
                $project = $this->db->query('SELECT * FROM projects WHERE id = ? FOR UPDATE', [$projectId])->getRowArray();
            ...
            if ($milestoneId) {
                $milestone = $this->db->query(
                    'SELECT milestones.*, projects.client_id AS project_client_id FROM milestones LEFT JOIN projects ON projects.id = milestones.project_id WHERE milestones.id = ? FOR UPDATE',
                    [$milestoneId]
                )->getRowArray();
            ...
            if ($invoice) {
                ...
                if ($amount > $outstanding) throw new \InvalidArgumentException('Payment amount cannot exceed the invoice balance of ' . number_format($outstanding, 2) . '.');
            }
