...
class PaymentController extends BaseController
{
    ...
    public function store(){
        if(!$this->validate(['client_id'=>'required|integer','amount'=>'required|decimal|greater_than[0]','method'=>'required|in_list[razorpay,upi,bank_transfer,cash,cheque]','payment_date'=>'required|valid_date'])) return redirect()->back()->withInput()->with('errors',$this->validator->getErrors());
        $invoiceId=$this->request->getPost('invoice_id')?:null; $projectId=$this->request->getPost('project_id')?:null; $milestoneId=$this->request->getPost('milestone_id')?:null; $transactionId=trim((string)($this->request->getPost('transaction_id')?:'')); $amount=(float)$this->request->getPost('amount'); $clientId=(int)$this->request->getPost('client_id');
        $this->db->transBegin();
        try{
            ...
            if($milestoneId){ $milestone=$this->db->query('SELECT milestones.*, projects.client_id AS project_client_id FROM milestones LEFT JOIN projects ON projects.id=milestones.project_id WHERE milestones.id=? FOR UPDATE',[$milestoneId])->getRowArray(); if(!$milestone)throw new \RuntimeException('Linked milestone not found.'); if((int)$milestone['project_client_id']!==$clientId)throw new \RuntimeException('Milestone does not belong to the selected client.'); if($projectId&&(int)$milestone['project_id']!==(int)$projectId)throw new \RuntimeException('Milestone does not belong to the selected project.'); }
            ...
            $pid=$this->pm->insert(['payment_number'=>$payNo,'client_id'=>$clientId,'project_id'=>$projectId,'invoice_id'=>$invoiceId,'milestone_id'=>$milestoneId,'amount'=>$amount,'method'=>$this->request->getPost('method'),'transaction_id'=>$transactionId?:null,'payment_date'=>$this->request->getPost('payment_date'),'notes'=>$this->request->getPost('notes')?:null,'status'=>'completed','created_by'=>session()->get('user_id')]);
